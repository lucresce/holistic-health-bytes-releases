...
**Última versão documentada neste README:** `latest`  
...
```bash
export HHB_VERSION=latest   # ou a tag do latest release
...
```bash
export HHB_VERSION=latest   # nova tag
docker pull ghcr.io/lucresce/holistic-health-bytes:${HHB_VERSION}
...
| Tag | Uso |
|-----|-----|
| `ghcr.io/lucresce/holistic-health-bytes:latest` | versão pinada |
...
```bash
docker pull ghcr.io/lucresce/holistic-health-bytes:latest
```
